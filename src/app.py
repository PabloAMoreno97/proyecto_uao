...
class App:
    ...
    def create_widgets(self):
        # LABELS
        self.lab1 = ttk.Label(self.root, text="Imagen Radiográfica", font=self.fonti)
        self.lab2 = ttk.Label(self.root, text="Imagen con Heatmap", font=self.fonti)
        self.lab3 = ttk.Label(self.root, text="Resultado:", font=self.fonti)
        self.lab4 = ttk.Label(self.root, text="Cédula Paciente:", font=self.fonti)
        self.lab5 = ttk.Label(self.root, 
                              text="SOFTWARE PARA EL APOYO AL DIAGNÓSTICO MÉDICO DE NEUMONÍA", 
                              font=self.fonti)
        self.lab6 = ttk.Label(self.root, text="Probabilidad:", font=self.fonti)
        
        # Etiquetas para mostrar la imagen y el heatmap
        self.image_label_frame = ttk.Frame(self.root, borderwidth=2, relief="sunken")
        self.image_label_frame.place(x=65, y=90, width=255, height=255)
        self.image_label = ttk.Label(self.image_label_frame)
        self.image_label.pack(fill="both", expand=True)

        self.heatmap_label_frame = ttk.Frame(self.root, borderwidth=2, relief="sunken")
        ...
        # Entradas de texto
        self.id_entry = ttk.Entry(self.root, textvariable=self.patient_id, width=20)
        self.result_text = tk.Text(self.root, width=15, height=1)
        self.proba_text = tk.Text(self.root, width=15, height=1)
        
        # Botones
        self.predict_button = ttk.Button(self.root, text="Predecir", state="disabled", command=self.run_model)
        self.load_button = ttk.Button(self.root, text="Cargar Imagen", command=self.load_image)
        self.clear_button = ttk.Button(self.root, text="Borrar", command=self.clear_ui)
        self.pdf_button = ttk.Button(self.root, text="Generar PDF", state="disabled", command=self.create_pdf)
        self.save_button = ttk.Button(self.root, text="Guardar en CSV", state="disabled", command=self.save_results_csv)
    ...
    def load_image(self):
        ...
        self.filepath = filedialog.askopenfilename(
            ...
            filetypes=[("DICOM Files", "*.dcm"), ("Image Files", "*.png;*.jpg;*.jpeg")]
        )
        
        if self.filepath:
            try:
                img_array, _ = read_dicom_file(self.filepath)
                self.original_image_pil = Image.fromarray(img_array).resize((250, 250), Image.LANCZOS)
                ...
                self.clear_ui(ask_confirm=False)
                showinfo(title="Error", message=f"No se pudo cargar la imagen.\nError: {e}")
    ...
    def run_model(self):
        ...
        if self.result_label:
            ...
            self.heatmap_pil = Image.fromarray(heatmap_array).resize((250, 250), Image.LANCZOS)
            ...
            showinfo(title="Error", message="La predicción falló. Revisa la consola.")
    ...
    def save_results_csv(self):
        ...
        with open(filepath, "a", newline="") as csvfile:
            ...
            writer.writerow([self.patient_id.get(), self.result_label, f"{self.probability:.2f}%", os.path.basename(self.filepath)])
            
        showinfo(title="Guardar", message=f"Los datos se guardaron en:\n{filepath}")

    def create_pdf(self):
        if not self.result_label:
            showinfo(title="PDF", message="Primero debe realizar una predicción.")
            return
        ...
        try:
            ...
            import pyautogui
            screenshot = pyautogui.screenshot(region=(self.root.winfo_x(), self.root.winfo_y(), self.root.winfo_width(), self.root.winfo_height()))
            
            # Guardamos la captura como un archivo temporal y la convertimos a PDF
            ...
            showinfo(title="PDF", message=f"El PDF fue generado en:\n{pdf_path}")
            
        except Exception as e:
            showinfo(title="Error", message=f"No se pudo generar el PDF.\nError: {e}")
