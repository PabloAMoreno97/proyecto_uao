...
def read_jpg_file(path):
    ...
    Returns:
        tuple: Una tupla con el array de la imagen y un objeto de
               imagen para mostrar.
    """
    img = cv2.imread(path)
    if img is None:
        raise FileNotFoundError(("No se pudo leer el"
                                 f"archivo de imagen: {path}"))
    img_array = np.asarray(img)
    img2_show = Image.fromarray(img_array)
...
def read_dicom_file(path):
    ...
    Returns:
        tuple: Una tupla con el array de píxeles y
               un objeto de imagen para mostrar.
    """
